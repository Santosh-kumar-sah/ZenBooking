...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-slate-100 font-sans">
      <header className={`fixed left-0 right-0 z-50 transition-all ${scrolled ? 'backdrop-blur-xl bg-surface-900/70 border-b border-white/6' : 'bg-transparent'}`}>
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-8 lg:flex">
            ...
            <a href="#for-business" className="text-sm text-slate-300 hover:text-white">For Business</a>
          </div>

          <div className="hidden items-center gap-3 lg:flex">
            <Link to={routes.login} className="inline-flex h-10 items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 text-sm text-slate-200">Login</Link>
          ...
          <button className="lg:hidden rounded-full p-2 border border-white/10 bg-white/5" onClick={() => setMenuOpen((s) => !s)} aria-label="menu">{menuOpen ? '✕' : '☰'}</button>
        </nav>

        {menuOpen && (
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="lg:hidden border-t border-white/10 bg-surface-900/80 px-4 py-4">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3 pt-2">
                <Link to={routes.login} className="flex-1 rounded-full border border-white/10 bg-white/5 py-2 text-center">Login</Link>
      ...
      <main className="pt-20">
        <section className="relative min-h-screen overflow-hidden">
          ...
          <div className="mx-auto flex min-h-[80vh] max-w-7xl items-center justify-between gap-12 px-4 py-24 sm:px-6 lg:px-8">
            <div className="max-w-2xl space-y-6">
              ...
              <p className="text-lg text-slate-300">Your customers book in seconds, you focus on your work — let AI handle the rest.</p>
              <div className="flex gap-3">
                ...
                <a href="#how-it-works" onClick={(e) => { e.preventDefault(); document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth' }); }} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">See How It Works</a>
              </div>

              <div className="mt-6">
                <div className="relative inline-block animate-float">
                  <div className="w-[360px] rounded-2xl border border-white/10 bg-white/[0.04] p-4 shadow-glow">
                    <div className="mb-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-semibold text-white">Zen Salon</div>
                          <div className="text-xs text-slate-400">Barbershop • 4.9⭐</div>
                        </div>
                      </div>
                      <div className="text-xs text-slate-400">Today</div>
                    </div>
                    <div className="grid grid-cols-7 gap-2 text-xs">
                      {['M','T','W','T','F','S','S'].map((d, i) => <div key={i} className="flex h-8 items-center justify-center rounded bg-white/3 text-slate-200">{d}</div>)}
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      <div className="rounded-full bg-white/5 px-3 py-2 text-sm text-white/90">10:00 AM</div>
                      <div className="rounded-full bg-gradient-to-r from-primary-500 to-accent-500 px-3 py-2 text-sm text-white">11:00 AM</div>
                      <div className="rounded-full bg-white/5 px-3 py-2 text-sm text-white/90">12:30 PM</div>
                    </div>
            ...
            <div className="hidden w-1/2 shrink-0 md:block">
              <div className="rounded-3xl border border-white/6 bg-surface-900 p-8 shadow-glow">
                <h3 className="text-lg font-semibold text-white">Quick overview</h3>
                <p className="mt-2 text-sm text-slate-400">Manage bookings, see insights and let AI handle reminders.</p>
                <div className="mt-6 grid gap-4">
                  <div className="flex items-center justify-between rounded-xl border border-white/6 bg-white/[0.02] p-4">
                    <div>
                      <div className="text-sm text-slate-400">Bookings today</div>
        ...
        <section id="features" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-extrabold text-white">Everything you need to run your bookings</h2>
            <p className="mt-3 text-sm text-slate-400">Manage schedule, automate reminders, and get AI insights — all in one place.</p>
          </div>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((f, idx) => {
              const Icon = f.icon;
              return (
                <motion.div key={f.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }} viewport={{ once: true }} className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <section id="how-it-works" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-extrabold text-white">Up and running in 3 steps</h2>
            <p className="mt-3 text-sm text-slate-400">Create your account, share a link, let customers book — it’s that simple.</p>
          </div>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {[{
              ...
              <motion.div key={s.n} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} viewport={{ once: true }} className="flex flex-col items-start gap-4 rounded-2xl border border-white/10 bg-white/5 p-6">
                <div className="flex items-center gap-3">
        ...
        <section id="for-business" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-2">
            <div>
              <h2 className="text-3xl font-extrabold text-white">Your personal AI receptionist</h2>
              <p className="mt-3 text-sm text-slate-400">Automate bookings and reminders so you never miss a customer.</p>
            ...
            <div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-glow">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-slate-400">Today's Overview</div>
                    <div className="mt-2 text-xl font-bold text-white">12 bookings • 3 upcoming</div>
                  </div>
                </div>
        ...
        <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-extrabold text-white">Trusted by businesses everywhere</h2>
            <p className="mt-3 text-sm text-slate-400">Real stories from real customers.</p>
          </div>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[{
              ...
              <div key={t.name} className="rounded-2xl border border-white/10 bg-white/5 p-6">
                ...
                <p className="mt-4 text-sm italic text-slate-200">“{t.quote}”</p>
              </div>
            ))}
          </div>
        </section>
